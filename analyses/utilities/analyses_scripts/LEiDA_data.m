...
%
% For each subject compute the leading eigenvector of the phase coherence
...
Data_info = [dir(fullfile(data_dir, '*.mat')); dir(fullfile(data_dir, '*.1D')); dir(fullfile(data_dir, '*.txt'))]; % CHANGED BY JL TO MAKE IT MORE GENERAL
...
for s = 1:n_scans
    ...
    signal = importdata(fullfile(data_dir, Data_info(s).name)); % CHANGED BY JL TO MAKE IT MORE GENERAL
    if isstruct(signal)
    ...
    if any(isnan(signal(:))) || any(isinf(signal(:))) || any(all(signal == 0,2))
        ...
        orig_filename = Data_info(s).name; % added by J/L
        index_name = strfind(orig_filename, 'task-')-1; % added by J/L
        saveHilber_Dir = fullfile(data_dir, 'hilbert_transform_phase');% added by J/L
        
        if ~exist(saveHilber_Dir, 'dir') % added by J/L
            mkdir(saveHilber_Dir); % added by J/L
        end
        
        save(fullfile(saveHilber_Dir, [orig_filename(1:index_name) 'hilbert_transform_phase.mat']), 'signal') % added by J/L
...
save(fullfile(save_dir, save_file), 'V1_all','Time_sessions','Data_info','idx_data') %CHANGED BY JL TO MAKE IT MORE GENERAL

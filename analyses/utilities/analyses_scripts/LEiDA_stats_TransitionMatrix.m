...
if isfile(fullfile(data_dir, file_V1)) % changed by JL to make it more general
    load(fullfile(data_dir, file_V1), 'Time_sessions', 'idx_data');
end
...
if isfile(fullfile(data_dir, file_P)) % changed by JL to make it more general
    load(fullfile(data_dir, file_P), 'cond', 'P', 'Index_Conditions', 'pair');
end
...
if pair == 0
    save(fullfile(save_dir, save_file),'TM','TMnorm','TM_pval','TM_pval2sided','effectsize','levene_pval',...,  % changed by JL to make it more general and not require specification of '/'
                              'cond','rangeK','file_cluster','file_V1','Index_Conditions') % JL ADDED TM_pval2sided because it is loaded on plot_k_diffs_transitions
else
    save(fullfile(save_dir, save_file),'TM','TMnorm','TM_pval','TM_pval2sided', 'effectsize',...,  % changed by JL to make it more general and not require specification of '/'
                              'cond','rangeK','file_cluster','file_V1','Index_Conditions') % JL ADDED TM_pval2sided because it is loaded on plot_k_diffs_transitions
end

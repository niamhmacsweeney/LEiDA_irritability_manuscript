function Plot_K_boxplot_DT(data_dir,save_dir,selectedK,parcellation)
%
% Plot the boxplot of dwell time (DT) values by condition for each of
% the centroids given by the specified K.
%
% INPUT:
...
%
% OUTPUT:
% .fig/.png     Boxplot of the dwell time values for each condition across
%               centroids
%
...
if isfile(fullfile(data_dir, file_LT)) % changed by JL to make it more general
    load(fullfile(data_dir, file_LT), 'cond', 'LT', 'LT_pval2sided', 'Index_Conditions');
end
...
%V = Kmeans_results{rangeK == selectedK}.C; %commented by JL

% Number of areas considered for analysis
%n_areas = size(V,2); %commented by JL
...
disp(' ');
disp(['Plotting the boxplot of dwell time values for each of the ' num2str(selectedK) ' PL states by condition:'])
...
for c = 1:selectedK
    ...
    plot(mean(X_locations(asterisks,:),2), Y_LOCATIONS(asterisks)*1.025, '*','Color','g','Markersize',4)
    ...
    plot(mean(X_locations(asterisks,:),2), Y_LOCATIONS(asterisks)*1.025, '*b','Markersize',4)
...
saveas(Fig, fullfile(save_dir, ['K' num2str(selectedK) '_BoxplotDwellTime.png']),'png');
saveas(Fig, fullfile(save_dir, ['K' num2str(selectedK) '_BoxplotDwellTime.fig']),'fig');
disp(['- Plot successfully saved as K' num2str(selectedK) '_BoxplotDwellTime']);

...
if isfile(fullfile(data_dir, file_cluster)) % changed by JL to make it more general
    load(fullfile(data_dir, file_cluster), 'Kmeans_results', 'rangeK'); % changed by JL to make it more general
end
...
sz = size(Volume);
% MNI_coord = zeros(max(Volume(:)),3); % commented by JL
MNI_coord = zeros(parcellation.n_areas,3); % added by JL

%for n = 1:max(Volume(:))% commented by JL
for n = 1:length(parcellation.areas) % added by JL
    %ind_N = find(Volume == n);% commented by JL
    ind_N = find(Volume == parcellation.areas(n));% added by JL

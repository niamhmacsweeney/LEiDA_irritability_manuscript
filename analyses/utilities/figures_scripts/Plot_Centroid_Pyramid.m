function Plot_Centroid_Pyramid(data_dir,cond,parcellation,cortex_dir)
%
...
% Load required data:
load(fullfile(data_dir, file_clusters),'Kmeans_results', 'rangeK'); %CHANGED BY JL TO MAKE TI MORE GENERAL
...
if n_Cond == 2 % Load 2-sided pvals from FracOccup hypothesis tests
    if isfile(fullfile(data_dir, file_FracOccup)) %CHANGED BY JL TO MAKE TI MORE GENERAL
        load(fullfile(data_dir, file_FracOccup), 'P_pval2sided'); %CHANGED BY JL TO MAKE TI MORE GENERAL
...
for k = 1:length(rangeK)
    ...
    for Centroid = 1:rangeK(k)
        ...
        for n = find(V >= 0)
            % Centroid_Vol(Volume == n) = 1;%commented by JL
            Centroid_Vol(Volume == parcellation.areas(n)) = 1; % added by JL
        end 

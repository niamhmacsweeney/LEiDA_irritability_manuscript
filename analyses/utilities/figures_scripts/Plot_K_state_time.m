...
% Load required data:
if isfile(fullfile(data_dir, file_V1))
    load(fullfile(data_dir, file_V1), 'Time_sessions', 'idx_data');  % changed by JL to make it more general
end
...
if isfile(fullfile(data_dir,file_P)) % changed by JL to make it more general
    load(fullfile(data_dir, file_P), 'cond', 'Index_Conditions');
end

...
if isfile(fullfile(data_dir, file_P)) % changed by JL to make it more general
    load(fullfile(data_dir, file_P), 'cond', 'P', 'P_pval2sided', 'Index_Conditions');
end
if isfile(fullfile(data_dir, file_LT)) % changed by JL to make it more general
    load(fullfile(data_dir, file_LT), 'LT', 'LT_pval2sided');
end
...
%Yeo_names = {'Visual','Somatomotor','Dorsal Att.','VentralAtt.','Limbic','Frontoparietal','Default Mode'}; % commented by JL
Yeo_struct = load('ParcelsMNI2mm',parcellation.YeoAtlas); % added by JL to make it more general
Yeo_names = Yeo_struct.(parcellation.YeoAtlas).labels; % added by JL to make it more general
clear Yeo_struct % added by JL to make it more general
...
[cc_V_yeo7, p_V_yeo7] = Overlap_LEiDA_Yeo(parcellation,Kmeans_results,rangeK);
clear Kmeans_results

% LEiDA networks colored according to closest RSN
%Original script: Volume = struct2array(load('ParcelsMNI2mm',['V_' parcellation]));
Volume_struct = load('ParcelsMNI2mm',parcellation.name); % added by JL/LK
Volume = Volume_struct.(parcellation.name).volume; % added by JL/LK
...
for c = 1:selectedK
    ...
    if numel(n_pos) > 0
        for region = 1:length(n_pos)
            %region_Vol = Volume == n_pos(region); % 3D volume with 1 only in the voxels of that area % commented by JL
            region_Vol = Volume == parcellation.areas(n_pos(region)); % 3D volume with 1 only in the voxels of that area % added by JL

...
if isfile(fullfile(data_dir, file_V1)) % changed by JL to make it more general
    load(fullfile(data_dir, file_V1), 'Time_sessions', 'Data_info', 'idx_data');
end
...
for i = 1:length(idx_data)
    ...
    if contains(Data_info(s).name,string(subj))
        signal = importdata(fullfile(Data_info(s).folder, Data_info(s).name));
        subj_name = Data_info(s).name;
        if isstruct(signal)
        % fMRI data should be stored in a field called data
        signal = signal.data; %commented by J/L 
        %signal = signal.timeseries_brainnetome_cortical_atlas; % added by J/L - script expects data to be called 'data', rename to match name from timeseries extraction script 
        end

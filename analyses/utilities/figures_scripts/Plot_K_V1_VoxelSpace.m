...
K_dir = fullfile(leida_dir, ['K' num2str(selectedK)]); % changed by JL to make it more general

if ~exist(K_dir, 'dir') % changed by JL to make it more general
    mkdir(K_dir);
end
...
% Load required data:
if isfile(fullfile(K_dir, file_V1_VoxelSpace))% changed by JL to make it more general
    load(fullfile(K_dir,file_V1_VoxelSpace), 'mean_V1', 'ind_voxels', 'MNI10mm_Mask');% changed by JL to make it more general
end
...
% Load required data:
if isfile(fullfile(leida_dir, file_cluster))
    load(fullfile(leida_dir, file_cluster), 'Kmeans_results', 'rangeK');
end
